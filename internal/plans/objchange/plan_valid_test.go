...
func TestAssertPlanValid(t *testing.T) {
	tests := map[string]struct {
		...
		// Nested unknown plan values need to be handled correctly. When
		// validating collection we often need to check length, which required
		// guarding for unknown values.
		"nested set attr planned unknown": {
			&configschema.Block{
				Attributes: map[string]*configschema.Attribute{
					"set": {
						Computed: true,
